...
describe('Department', () => {
  ...
  describe('Updating data', () => {
    ...
    it('should properly update multiple documents with "updateMany" method', async () => {
      ...
      expect(department[0].name).to.be.equal('Updated');
      expect(department[1].name).to.be.equal('Updated');
    });

    it('should properly update multiple documents with "updateMany" method #2', async () => {
      await Department.updateMany({}, { $set: { name: 'Updated' } });
      const department = await Department.find({ name: 'Updated' });
      expect(department.length).to.be.equal(2);
    });
